#!/usr/bin/env node
/**
 * Enterprise Wallet Manager MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own EWM API key as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  // Default: JSON
  res.json({
    name: 'Enterprise Wallet Manager MCP Server',
    provider: 'AgenticLedger',
    version: '1.0.0',
    description:
      'Manage enterprise crypto wallets, organizations, approvals, balances, and signatories through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your EWM API key as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-ewm-api-key>',
      howToGetKey:
        'Generate an API key from your Enterprise Wallet Manager dashboard.',
    },
    configTemplate: {
      mcpServers: {
        'enterprise-wallet-manager': {
          ...
          headers: {
            Authorization: 'Bearer <your-ewm-api-key>',
          },
        },
      },
    },
...
<head>
  ...
  <title>Enterprise Wallet Manager MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Enterprise Wallet Manager MCP Server</h1>
    <p class="subtitle">Manage enterprise crypto wallets, organizations, approvals, balances, and signatories through MCP tools.</p>

    <div class="card">
      <h2>How to get your EWM API key</h2>
      <ol class="steps">
        <li>Log in to your <a href="https://walletmanager.agenticledger.ai" target="_blank" rel="noopener">Enterprise Wallet Manager</a> dashboard.</li>
        <li>Navigate to <strong>Settings &rarr; API Keys</strong>.</li>
        <li>Click <strong>Generate new API key</strong>.</li>
        <li>Copy the key (starts with <code>ewm_live_</code>) and paste it below.</li>
      </ol>
    </div>

    <div class="card">
      <h2>Generate your MCP config</h2>
      <label for="apiKey">EWM API Key</label>
...
app.get('/health', (_req, res) => {
  res.json({
    status: 'ok',
    server: 'enterprise-wallet-mcp-http',
    version: '1.0.0',
    tools: tools.length,
    transport: 'streamable-http',
    auth: 'dual-mode',
...
function createMCPServer(client: EWMClient): Server {
  const server = new Server(
    { name: 'enterprise-wallet-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  const client = new EWMClient(apiKey);
...
<head>
  ...
  <title>Authorize \u2014 Enterprise Wallet Manager MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Enterprise Wallet Manager MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Enterprise Wallet Manager MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Enterprise Wallet Manager MCP HTTP Server running on port ${PORT}`);
